...
namespace KatlaSport.Services.HiveManagement
{
    ...
    public class HiveSectionService : IHiveSectionService
    {
        ...
        {
            var dbSections = await _context.Sections.Where(h => hiveSectionId == h.Id).ToArrayAsync();
        ...
        {
            var dbHivesSection = await _context.Sections.Where(h => h.Code == createRequest.Code).ToArrayAsync();
            if (dbHivesSection.Length > 0)
            {
        ...
        {
            var dbHivesSection = await _context.Sections.Where(h => h.Code == updateRequest.Code && h.Id != hiveSectionId).ToArrayAsync();
            if (dbHivesSection.Length > 0)
            {
                throw new RequestedResourceHasConflictException("code");
            }

            dbHivesSection = await _context.Sections.Where(h => h.Id == hiveSectionId).ToArrayAsync();
            var dbHiveSection = dbHivesSection.FirstOrDefault();
        ...
        {
            var dbHivesSection = await _context.Sections.Where(h => h.Id == hiveSectionId).ToArrayAsync();
            if (dbHivesSection.Length == 0)
            {
                throw new RequestedResourceNotFoundException();
            }

            var dbHiveSection = dbHivesSection[0];
